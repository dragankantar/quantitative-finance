...
data = pd.read_csv("stock_ret.csv", index_col="Date")
...
sim_frame = pd.DataFrame(simulation_res.T,columns=['ret','stdev','sharpe',data.columns[0],data.columns[1],data.columns[2],data.columns[3], \
                                                   data.columns[4],data.columns[5],data.columns[6],data.columns[7],data.columns[8]])
header='ret,stdev,sharpe'+','+data.columns[0]+','+data.columns[1]+','+data.columns[2]+','+data.columns[3]+','+ \
        data.columns[4]+','+data.columns[5]+','+data.columns[6]+','+data.columns[7]+','+data.columns[8]
np.savetxt('sim_frame.csv', sim_frame, fmt='%.8f', delimiter=',', comments='', header=header)
